...
testList = makeUsableList(wireList)
...
def gateCornerPiece(Gate, corner, gateDirection, size):
...
# corner = [x,y,z, 'from', 'to']
def CornerCornerPiece(StartCorner, EndCorner, size):
    startpoints = GetPoints(StartCorner, size)
    endpoints = GetPoints(EndCorner, size)
    #make 4 squares
...
def makeGateConnect(startingpoint, endpoint)
def makeLineUnit(startingpoint, endingpoint, size, cornerlist): #lists of 3 floats
    pass
    for segment in range(cornerlist):
        if segment != cornerlist[-1]:
            pass
